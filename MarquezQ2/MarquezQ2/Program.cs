...
//wm added using system.linq for method .Count()
using System.Linq;
...
namespace TShirtOrders
{
    class Program
    {
        ...
        {
            decimal total = 0;
            //wm changed (string order in orders) to  (TShirtOrder order in orders)
            foreach (TShirtOrder order in orders)
            {
                total += order.Price;
            }
            //wm formatted total as "c" for currancy
            Console.WriteLine("Total price of order is: " + total.ToString("c"));
        }
        ...
        private static bool GetBoolFromUser(string prompt)
        {
            ...
            //wm cleaned up input with Trim().ToLower
            string input = Console.ReadLine().Trim().ToLower();
            //wm check for something enterd
